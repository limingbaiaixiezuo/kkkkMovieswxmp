...
CREATE TABLE `review` (
  `id` int(11) NOT NULL COMMENT '影片ID',
  `user` varchar(255) DEFAULT NULL COMMENT '影评的发布者',
...
SET @IMAGE_BASE_URL = "https://film-1258458885.cos.ap-beijing.myqcloud.com/"; -- FOR EXAMPLE: https://*****.ap-shanghai.myqcloud.com/
...
--
-- 表的结构 `user_collect_review`
--
CREATE TABLE `user_collect_review` (
  `collect_id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'user收藏ID',
  `id` int(11) NOT NULL COMMENT '影片ID',
  `review_id` int(11) NOT NULL COMMENT '影评ID',
  `user` varchar(255) DEFAULT NULL COMMENT '收藏者——本账户拥有者(限定本地用户)',
...
ALTER TABLE `review`
  ADD CONSTRAINT `review_link` FOREIGN KEY (`id`) REFERENCES `movies` (`id`);


--
-- 限制表 `user_collect_review`
--

ALTER TABLE `user_collect_review`
  ADD CONSTRAINT `user_collect_link1` FOREIGN KEY (`id`) REFERENCES `movies` (`id`),
  ADD CONSTRAINT `user_collect_link2` FOREIGN KEY (`review_id`) REFERENCES `review` (`review_id`);
